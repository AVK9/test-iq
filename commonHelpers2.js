...
`;function M(e){const i=document.getElementById("progress-bar"),c=e/b.length*100;i.style.width=`${c}%`}const N="/test-iq/assets/call-BdpohKNc.svg",$=x.create({baseURL:"https://swapi.dev/api/people/1/"}),A=async()=>{const{data:e}=await $.get("");return e},H=`<div class="rezult-container container">
...
</div>`;let g;function S(e,i){let c=e,a,t;g=setInterval(()=>{a=Math.floor(c/60),t=c%60,a=a<10?"0"+a:a,t=t<10?"0"+t:t,i.textContent=a+":"+t,--c<0&&(clearInterval(g),i.textContent="00:00"),document.getElementById("btn-coll").addEventListener("click",()=>O(i))},1e3)}function O(e){clearInterval(g),e.textContent="00:00"}function z(){A().then(e=>{P(e)}).catch(e=>{console.log(e)})}function P(e){const i=document.getElementById("person-info");i.innerHTML=`
  <ul>
...
`,f=document.getElementById("btn-burger"),K=document.getElementById("popup-container"),C=document.getElementById("btn-popup-close");document.addEventListener("DOMContentLoaded",()=>{f&&f.addEventListener("click",w),C&&C.addEventListener("click",w)});function w(){K.classList.toggle("show")}const h=document.getElementById("home-container");h.innerHTML=V;document.addEventListener("DOMContentLoaded",()=>{const e=document.getElementById("home-btn-more");e&&e.addEventListener("click",j)});function j(){h.insertAdjacentHTML("beforeend",G);const e=document.getElementById("del-home-btn-more");e.innerHTML=""}document.addEventListener("DOMContentLoaded",()=>{const e=document.getElementById("home-btn-test");e&&e.addEventListener("click",J)});function J(){h.innerHTML="",h.innerHTML=Z,y(),document.getElementById("next-button").addEventListener("click",D)}
