...
`;function B(e){const i=document.getElementById("progress-bar"),c=e/h.length*100;i.style.width=`${c}%`}const q="/test-iq/assets/call-BdpohKNc.svg",T=C.create({baseURL:"https://swapi.dev/api/people/1/"}),M=async()=>{const{data:e}=await T.get("");return e},k=`<div class="rezult-container">
...
</div>`;let g;function N(e,i){let c=e,a,t;g=setInterval(()=>{a=Math.floor(c/60),t=c%60,a=a<10?"0"+a:a,t=t<10?"0"+t:t,i.textContent=a+":"+t,--c<0&&(clearInterval(g),i.textContent="00:00"),document.getElementById("btn-coll").addEventListener("click",()=>A(i))},1e3)}function A(e){clearInterval(g),e.textContent="00:00"}function H(){M().then(e=>{$(e)}).catch(e=>{console.log(e)})}function $(e){const i=document.getElementById("person-info");i.innerHTML=`
  <ul>
...
`,b=document.getElementById("home-container");b.innerHTML=R;document.addEventListener("DOMContentLoaded",()=>{const e=document.getElementById("home-btn-more");e&&e.addEventListener("click",_)});function _(){b.insertAdjacentHTML("beforeend",U);const e=document.getElementById("del-home-btn-more");e.innerHTML=""}document.addEventListener("DOMContentLoaded",()=>{const e=document.getElementById("home-btn-test");e&&e.addEventListener("click",F)});function F(){b.innerHTML="",b.innerHTML=z,f(),document.getElementById("next-button").addEventListener("click",O)}
